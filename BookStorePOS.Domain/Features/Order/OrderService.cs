    ...
    public OrderCreateResponseModel CreateOrder(OrderCreateRequestModel requestModel)
    {
        try
        {
            ...
            foreach (var item in requestModel.Items)
            {
                ...
                // Add directly to the navigation property instead of saving early
            ...
            // EF Core will save the Order, all OrderItems, and Book stock updates in one implicit transaction
            _db.Orders.Add(order);
            _db.SaveChanges();

            // Populate the IDs generated by the database back into the response model
            int i = 0;
            foreach (var dbItem in order.OrderItems)
            {
                orderModelItems[i].OrderItemId = dbItem.OrderItemId;
